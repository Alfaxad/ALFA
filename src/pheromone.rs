...
use crate::config::{PH_UNIT_GRID_SIZE, MAX_PHEROMONE_STRENGTH};
...
impl Plugin for PheromonePlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, setup)
            ...
            .add_systems(
                Update,
                update_kd_tree.run_if(on_timer(Duration::from_secs_f32(PH_KD_TREE_UPDATE_INTERVAL))),
            )
            .add_systems(
                Update,
                update_sim_stats.run_if(on_timer(Duration::from_secs_f32(PH_KD_TREE_UPDATE_INTERVAL))),
            )
            .add_systems(
                Update,
...
impl Pheromones {
    fn new() -> Self {
        ...
        to_home_map.insert((HIVE_LOCATION.0 as i32, HIVE_LOCATION.1 as i32), 100000.0);
